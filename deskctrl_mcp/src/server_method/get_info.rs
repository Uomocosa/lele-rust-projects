    ...
    ServerInfo::new(ServerCapabilities::builder().enable_tools().build())
        ...
        .with_instructions(
            "deskctrl: desktop and process control. Windows: list_windows (open windows \
              ...
              VPN: check_vpn_status (protonvpn status + public IP), activate_vpn \
              (protonvpn connect, no sudo, prefer protonvpn binary), deactivate_vpn \
              (protonvpn disconnect). RULE: every visible-action tool (screenshot, \
              click_window, send_keys, spawn_process, write_stdin, kill_process, \
              record_video) accepts send_to_telegram, which defaults to true. Leave it true \
              only for steps with visible impact (clicks, captures, spawns); set it false \
              for routine/read-only calls. Each notified action goes out as its own short \
              step-by-step Telegram message (screenshots include the image with a caption).",
        )
}
