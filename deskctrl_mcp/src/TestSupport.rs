//! Shared fixtures for live-system tests (real X display, real Telegram sends). These assert
//! their prerequisite is actually present rather than silently skipping — a live test with a
//! missing prerequisite should fail loudly, naming what's missing, not report `ok`.
