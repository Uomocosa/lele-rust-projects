...
fn decode_params(params: &[u8]) -> BTreeMap<u64, Pubkey> {
    if params.is_empty() {
        return BTreeMap::new();
    }
    bincode::deserialize::<BTreeMap<u64, Pubkey>>(params).unwrap_or_default()
}
...
fn verify_sig(pubkey: &Pubkey, tag: u64, value: u64, sig_bytes: &[u8]) -> bool {
    ...
    let msg = bincode::serialize(&(tag, value)).unwrap_or_default();
...
impl ContractInterface for GlobalCounterContract {
    ...
    fn update_state(
        ...
        for update in data {
            match update {
                UpdateData::State(s) => {
                    let Ok(signed) = decode_signed_slots(s.as_ref()) else {
                        continue;
                    };
                    for (tag, value) in signed.slots {
                        ...
                        let Some(pubkey) = allow_list.get(&tag) else {
                            continue;
                        };
                        let Some(sig) = signed.sigs.get(&tag) else {
                            continue;
                        };
                        if !verify_sig(pubkey, tag, value, sig) {
                            continue;
                        }
...
mod tests {
    ...
    fn params_for_tags(tags: &[u64]) -> Parameters<'static> {
        ...
        for &t in tags {
            ...
            m.insert(t, *vk.as_bytes());
        }
        Parameters::from(bincode::serialize(&m).unwrap())
    }

    fn signed_state(slots: &[(u64, u64)], tags: &[u64]) -> State<'static> {
        let mut sigs = BTreeMap::new();
        for &(tag, value) in slots {
            let mut seed = [0u8; 32];
            seed[0..8].copy_from_slice(&tag.to_le_bytes());
            let sk = SigningKey::from_bytes(&seed);
            let msg = bincode::serialize(&(tag, value)).unwrap();
            let sig = sk.sign(&msg);
            sigs.insert(tag, sig.to_bytes().to_vec());
        }
        for &t in tags {
            if !slots.iter().any(|(k, _)| *k == t) {
                continue;
            }
        }
        let map: BTreeMap<u64, u64> = slots.iter().copied().collect();
        let ss = SignedSlots { slots: map, sigs };
    ...
    fn test_usage() {
        ...
        let update = vec![UpdateData::State(signed_state(&[(1, 7)], &[1]))];
        let result =
            GlobalCounterContract::update_state(params(), state.clone(), update).unwrap();
    ...
    fn window_blocks_large_jump() {
        ...
        let max_jump = vec![UpdateData::State(signed_state(&[(0, 1_000_000)], &[0]))];
        ...
        assert_eq!(decoded.get(&0), Some(&5), "MAX jump should be blocked by window");

        let ok_step = vec![UpdateData::State(signed_state(&[(0, 9)], &[0]))];
        let result = GlobalCounterContract::update_state(p.clone(), state.clone(), ok_step).unwrap();
        let next = result.unwrap_valid();
        let decoded = bincode::deserialize::<Slots>(next.as_ref()).unwrap();
        assert_eq!(decoded.get(&0), Some(&9), "value within window +5 should be accepted");

        let bad_sig = {
            let mut ss = SignedSlots {
                slots: BTreeMap::from([(0, 6)]),
                sigs: BTreeMap::new(),
            };
            ss.sigs.insert(0, vec![0u8; 64]);
            State::from(bincode::serialize(&ss).unwrap())
        };
        let result =
            GlobalCounterContract::update_state(p, state, vec![UpdateData::State(bad_sig)]).unwrap();
        let next = result.unwrap_valid();
        let decoded = bincode::deserialize::<Slots>(next.as_ref()).unwrap();
        assert_eq!(decoded.get(&0), Some(&5), "bad sig should be rejected");
    }
}
