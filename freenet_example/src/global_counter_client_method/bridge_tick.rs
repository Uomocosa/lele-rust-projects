...
// needed helper:
const fn response_kind(response: &HostResponse) -> &'static str {
    match response {
...
mod tests {
    ...
    fn test_usage() {
        ...
        let Some(stale) = now.checked_sub(Duration::from_secs(120)) else {
            return;
        };
        assert!(bridge_due(Some(stale), None, now));
        assert!(bridge_due(Some(stale), Some(stale), now));

        let Some(fresh) = now.checked_sub(Duration::from_secs(1)) else {
            return;
        };
