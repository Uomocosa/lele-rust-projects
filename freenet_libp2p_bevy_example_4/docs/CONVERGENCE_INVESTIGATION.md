...
The intermittent "convergence failure" of `mainnet_automation_4` is **not primarily a Freenet
...
1. **Fresh contract key every run.** `mainnet_automation_4/src/new_contract_params.rs` generates a
...
`mainnet_automation_4` end-to-end **PASSED**: all 3 instances mutually converged (each saw 2 peers),
...
- [ ] Consider matching the automation gate to the fixed behaviour and re-running the full
      `mainnet_automation_4` harness end-to-end to confirm the gate passes.