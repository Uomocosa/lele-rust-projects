...
**Root cause found and fixed (see RESULT).** Phase 1 diagnostics + clean log analysis proved the
intermittent convergence failure was **not Freenet** and **not** a roster/discovery problem. It was
a **libp2p `request_response` misuse** in our netcode: `src/p2p/run.rs` never responded to inbound
requests, so pending requests accumulated and the netcode *receive* leg stalled to zero while
outbound kept working. Fixing `run.rs` to reply `Ack` to every inbound request restored
bidirectional netcode and full 3-instance convergence (roster reached 3/3, ~8k inbound messages
per instance, 0 request failures — verified by a live mainnet run).

The originally-planned delivery layers (RosterSync, gossipsub) are **no longer needed** — they were
compensating for a bug that did not exist in the discovery layer.

The Phase 1 diagnostic instrumentation, the two "on us" edge cases found during investigation
(fresh-key Put race, 5-min TTL prune), and the request-response fix are all documented in
`docs/CONVERGENCE_INVESTIGATION.md`.
...
1. **No local-special handling.** A `--bootstrap-roster` / automation-injected discovery path is
   ...
   cross-OS run.
2. **Discovery is fully Freenet-driven** (contract identity gate). No libp2p address injection.
...
### Phase 1 RESULT (executed 2026-08-24) — root cause found, not H1–H4 as framed

The diagnostic was run (3-instance mainnet, ~6 min, ANSI-stripped log analysis). Findings:

- **H1 contract identity — RULED OUT.** All 3 instances logged identical `contract_id`,
  `code_hash`, and `params_hex`.
- **Discovery is not the discriminator.** On prior "failed" runs the roster contract still reached
  **3/3**; Freenet discovery worked.
- **The real fault: netcode receive leg, from a `request_response` misuse.** Failing runs sent
  ~12k outbound snapshots but received **0** inbound netcode. Cause: `src/p2p/run.rs` never
  responded to inbound requests (`latest_netcode` stayed `None`), so unanswered requests
  accumulated and stalled the inbound direction.
- **Fix (verified):** reply `Ack` to every inbound request. A fresh run then gave all 3 instances
  full roster (3/3), ~8k inbound messages each, and **0** request failures — bidirectional netcode.

Full writeup and the prior `subscriber_peer_ids` red-herring are in
`docs/CONVERGENCE_INVESTIGATION.md`.

**Remaining (secondary, from investigation):** the fresh-key Put race and the 5-min TTL prune can
still cause roster regressions in some runs; see docs. Not the root cause of the netcode failures.

## Plan

The original delivery-layer phases (gossipsub/RosterSync, live-join catch-up, ring-health/retry)
were **superseded**: Phase 1 proved the failure was a netcode `request_response` bug, already fixed.
They remain below for reference only. The concrete remaining work is the Phase 2 netcode fix (done)
and, opportunistically, the secondary roster edge cases in the docs.
...
### Phase 2 (DONE — the actual root-cause fix): reply `Ack` to every netcode request

**Superseded the gossipsub/RosterSync plan.** Phase 1 proved the roster/discovery layer was fine
and the failure was the netcode receive leg caused by never answering libp2p `request_response`
requests.

**Change (landed in `src/p2p/run.rs`):** on every inbound netcode `Request`, reply
`NetcodeMsg::Ack` (previously `latest_netcode` was `None` forever, so no request was ever answered).
`NetcodeMsg::Ack` is a no-op in `netcode_tick.rs`. Diagnostic logging for request-response
outbound/inbound failures was added alongside.

**Verification:** a fresh 3-instance mainnet run reached roster 3/3 on all instances, exchanged
~8k inbound messages each, and logged **0** request failures.

**Original gossipsub design (not needed now, retained for reference):**
