        ...
        match event {
            p2p::Event::PeerConnected(peer_id) => {
                tracing::info!(
                    target: "p2p",
                    peer = %peer_id,
                    "peer connected, requesting snapshot"
                );
                commands
                    .send(p2p::Command::RequestSnapshot {
            ...
            p2p::Event::IncomingNetcode { from, msg } => {
                ...
                match msg {
                    ...
                    p2p::NetcodeMsg::RequestSnapshot { player_id } => {
                        tracing::info!(
                            target: "p2p",
                            from = ?from,
                            player = %hex::encode(player_id),
                            "received RequestSnapshot, spawning joiner + replying with state"
                        );
                        engine.send_cmd(engine::EngineCmd::Spawn(player_id));
                        let sim = engine.get_sim_state();
                        let participants: Vec<engine::PlayerId> = roster.keys().copied().collect();
                        let pos_bodies: std::collections::BTreeMap<engine::PlayerId, (f32, f32)> =
                            sim.bodies
                                .iter()
                                .map(|(pid, b)| (*pid, (b.0, b.1)))
                                .collect();
                        let snap = engine::Snapshot {
                            tick: sim.tick,
                            bodies: pos_bodies,
                        };
                        let hash = engine::hash_snapshot(&snap);
                        let msg = p2p::NetcodeMsg::Snapshot {
                            tick: sim.tick,
                            bodies: sim.bodies,
                            participants,
                            from: own,
                        ...
                        send(&commands, &from.to_base58(), msg);
                    }
                    p2p::NetcodeMsg::Snapshot {
                        ...
                        state.pending_snapshots.push(p2p::NetcodeMsg::Snapshot {
                            tick,
                            bodies,
                            participants,
                            from,
