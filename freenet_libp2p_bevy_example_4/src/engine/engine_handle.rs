...
impl EngineHandle {
    ...
    pub fn get_sim_state(&self) -> engine::EngineSimState {
        self.send_cmd(engine::EngineCmd::GetSimState);
        loop {
            if let Some(engine::EngineReply::SimState(state)) = self.recv_reply() {
                return state;
            }
        }
    }
