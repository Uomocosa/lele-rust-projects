...
pub async fn connect_with_retry(port: u16, wasm: &[u8], params: &[u8], tag: u64) -> ClickerClient {
    let mut attempt = 0u64;
    loop {
        attempt = attempt.wrapping_add(1);
        match ClickerClient::connect_with_tag("127.0.0.1", port, wasm, params, Role::Publish, tag)
            .await
        {
            Ok(c) => return c,
            Err(e) => {
