...
mod tests {
    ...
    fn test_usage() {
        ...
        base.insert(boxes::PlayerId(1), entry("peer-1", 5));

        let mut other = roster::RosterState::default();
        other.insert(boxes::PlayerId(1), entry("peer-1-stale", 1));
        other.insert(boxes::PlayerId(2), entry("peer-2", 9));

        let merged = merge_roster(base, other);

        assert_eq!(merged.len(), 2);
        assert_eq!(merged[&boxes::PlayerId(1)].peer_id, "peer-1");
        assert_eq!(merged[&boxes::PlayerId(2)].peer_id, "peer-2");
    }
}
