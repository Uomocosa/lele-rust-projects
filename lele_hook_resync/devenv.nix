{ pkgs, lib, config, inputs, ... }: {
  ...
  git-hooks.hooks = {
    lele-clippy = {
      enable = true;
      name = "clippy (lele_hook_resync)";
      entry = "bash -c 'cd lele_hook_resync && devenv tasks run lele:clippy 2>&1'";
      pass_filenames = false;
      always_run = true;
    };
    lele-fmt = {
      enable = true;
      name = "fmt (lele_hook_resync)";
      entry = "bash -c 'cd lele_hook_resync && devenv tasks run lele:fmt 2>&1'";
      pass_filenames = false;
      always_run = true;
    };
    lele-lint = {
      enable = true;
      name = "lele_lint (lele_hook_resync)";
      entry = "bash -c 'cd lele_hook_resync && devenv tasks run lele:lint 2>&1'";
      pass_filenames = false;
      always_run = true;
    };
    lele-taxonomy = {
      enable = true;
      name = "taxonomy_check (lele_hook_resync)";
      entry = "bash -c 'cd lele_hook_resync && devenv tasks run lele:taxonomy_check 2>&1'";
      pass_filenames = false;
      always_run = true;
    };
  };
