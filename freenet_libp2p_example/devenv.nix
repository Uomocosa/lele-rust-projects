{ pkgs, lib, config, inputs, ... }: {
  ...
  git-hooks.hooks = {
    lele-clippy = {
      enable = true;
      name = "clippy (freenet_libp2p_example)";
      entry = "bash -c 'cd freenet_libp2p_example && devenv tasks run lele:clippy 2>&1'";
      pass_filenames = false;
      always_run = true;
    };
    lele-fmt = {
      enable = true;
      name = "fmt (freenet_libp2p_example)";
      entry = "bash -c 'cd freenet_libp2p_example && devenv tasks run lele:fmt 2>&1'";
      pass_filenames = false;
      always_run = true;
    };
    lele-lint = {
      enable = true;
      name = "lele_lint (freenet_libp2p_example)";
      entry = "bash -c 'cd freenet_libp2p_example && devenv tasks run lele:lint 2>&1'";
      pass_filenames = false;
      always_run = true;
    };
    lele-taxonomy = {
      enable = true;
      name = "taxonomy_check (freenet_libp2p_example)";
      entry = "bash -c 'cd freenet_libp2p_example && devenv tasks run lele:taxonomy_check 2>&1'";
      pass_filenames = false;
      always_run = true;
    };
    freenet-contract-harness = {
      enable = true;
      name = "contract-harness (freenet_libp2p_example)";
      entry = "bash -c 'cd freenet_libp2p_example && devenv tasks run freenet:contract-harness 2>&1'";
      pass_filenames = false;
      always_run = true;
    };
  };
