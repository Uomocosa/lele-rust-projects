{ pkgs, lib, config, inputs, ... }: {
  ...
  git-hooks.hooks = {
    lele-clippy = {
      enable = true;
      name = "clippy (starboard)";
      entry = "bash -c 'cd starboard && devenv tasks run lele:clippy 2>&1'";
    ...
    lele-fmt = {
      enable = true;
      name = "fmt (starboard)";
      entry = "bash -c 'cd starboard && devenv tasks run lele:fmt 2>&1'";
      pass_filenames = false;
      always_run = true;
    };
    lele-lint = {
      enable = true;
      name = "lele_lint (starboard)";
      entry = "bash -c 'cd starboard && devenv tasks run lele:lint 2>&1'";
      pass_filenames = false;
      always_run = true;
    };
    lele-taxonomy = {
