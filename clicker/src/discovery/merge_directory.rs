...
pub fn merge_directory(
    mut base: discovery::DirectoryState,
    other: discovery::DirectoryState,
) -> discovery::DirectoryState {
    for (room, entry) in other {
        let merged = discovery::merge_directory_entry(base.remove(&room), entry);
...
mod tests {
    ...
    fn test_usage() {
        ...
        other.insert("room-b".to_string(), entry(1));
