...
fn dump(mesh: &mut testing::Mesh, one: usize) -> String {
    let Some(app) = mesh.apps.get_mut(one) else {
        return String::new();
    };
...
fn app_one(mesh: &testing::Mesh) -> usize {
    mesh.apps
        .iter()
...
fn unlabeled_clicks_park_exact() {
    ...
    let one = app_one(&mesh);
    for _ in 0..17 {
        testing::push_to(&mut mesh, 1, click_gossip("peer-3", 3));
    }
    for _ in 0..5 {
        testing::push_to(&mut mesh, 1, click_gossip("peer-2", 2));
    }
    testing::push_to(
        &mut mesh,
        1,
    ...
    assert_eq!(
        counts[one],
        testing::MeshCount::of([(testing::Player(2), 5), (testing::Player(3), 17),]),
