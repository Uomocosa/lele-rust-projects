...
async fn read_event(
    stream: &mut turmoil::net::TcpStream,
) -> std::io::Result<p2p::Event<clicker::CursorMsg>> {
...
async fn pump(
    ...
    inbox: &mut tokio::sync::mpsc::UnboundedReceiver<p2p::Event<clicker::CursorMsg>>,
) {
    app.update();
    let cmds = app
        .world_mut()
    ...
    for cmd in cmds {
        ...
        for target in targets {
            if let Some(stream) = outbound.get_mut(&target) {
                write_event(stream, &event).await.ok();
            }
        }
    }
...
async fn read_loop(
    ...
    inbox: tokio::sync::mpsc::UnboundedSender<p2p::Event<clicker::CursorMsg>>,
) {
    loop {
...
fn run_peer(
    ...
    async move {
                    ...
                    loop {
                        match turmoil::net::TcpStream::connect((peer, PORT)).await {
                            ...
                            Err(e) => {
                                ...
                                if attempts % 50 == 0 {
                                    eprintln!(
                                        "TURMOIL {name}: connect {peer} retry {attempts}: {e}"
                                    );
                                }
        ...
        accept_ok?;
        eprintln!("TURMOIL {name}: connected");
        ...
        for _ in 0..HANDSHAKE_ITERS {
            pump(&mut app, name, &mut outbound, &mut inbox_rx).await;
            if labeled(&mut app) >= 3 {
                break;
            }
            tokio::time::sleep(STEP_SLEEP).await;
        }
        eprintln!("TURMOIL {name}: handshake labeled={}", labeled(&mut app));
        testing::click_times(&mut app, clicks);
        eprintln!("TURMOIL {name}: clicked {clicks}");
        ...
        eprintln!("TURMOIL {name}: final {:?}", counts_of(&mut app));
...
#[test]
fn asymmetric_1_5_17_heals_after_hold() {
    ...
    let mut sim = turmoil::Builder::new()
        ...
        .build();
    for (slot, (name, own, clicks)) in [("peer-1", 1u64, 1u32), ("peer-2", 2, 5), ("peer-3", 3, 17)]
        .into_iter()
        .enumerate()
    {
        let seen = results.clone();
        sim.host(name, move || {
            run_peer(name, own, clicks, slot, seen.clone())
        });
    }
    let seen = results.clone();
