{ pkgs, lib, config, inputs, ... }: {
  ...
  tasks = {
    ...
    "ui:previews-telegram" = { exec = "export VK_ICD_FILENAMES=\"${pkgs.mesa}/share/vulkan/icd.d/radeon_icd.x86_64.json\" && cargo nextest run ui_png --features dev --run-ignored all -- --nocapture && cargo run --features dev --example send_previews"; showOutput = true; };
  };
