...
pub fn dial_hint_raw(
    ...
    if hint.peer_id.is_empty() || hint.peer_id.as_str() == peer_id || hint.addrs.is_empty() {
        return;
    }
    ...
    let age = attempted.get(entry.peer_id.as_str()).and_then(|seen| {
        Instant::now()
    ...
    match decide_dial(peer_id, entry.peer_id.as_str(), age) {
        Decision::Wait => {
            attempted
