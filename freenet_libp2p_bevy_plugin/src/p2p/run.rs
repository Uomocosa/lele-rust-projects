...
pub async fn run<T: p2p::Message>(
    ...
    let mut latest_snapshot: Option<p2p::Snapshot<T>> = None;
            ...
            event = swarm.select_next_some() => match event {
                ...
                SwarmEvent::Behaviour(p2p::behaviour::BehaviourEvent::Positions(
                    ...
                    request_response::Message::Request { request, channel, .. } => {
                        event_tx
                            .send(p2p::Event::IncomingSnapshot { from: peer, snapshot: request.clone() })
                            .ok();
                        if let Some(reply) = latest_snapshot.as_ref() {
                            swarm.behaviour_mut().positions.send_response(channel, reply.clone()).ok();
                        }
                    }
...
mod tests {
    ...
    async fn wait_ready(
        rx: &mut tokio::sync::mpsc::UnboundedReceiver<p2p::Event<Scan>>,
    ) -> (String, Vec<String>) {
        loop {
    ...
    async fn two_swarm_snapshot_exchange() {
        let (cmd_tx_a, cmd_rx_a) = tokio::sync::mpsc::unbounded_channel::<p2p::Command<Scan>>();
        let (event_tx_a, mut event_rx_a) =
            tokio::sync::mpsc::unbounded_channel::<p2p::Event<Scan>>();
        let (cmd_tx_b, cmd_rx_b) = tokio::sync::mpsc::unbounded_channel::<p2p::Command<Scan>>();
        let (event_tx_b, mut event_rx_b) =
            tokio::sync::mpsc::unbounded_channel::<p2p::Event<Scan>>();
        ...
        let b_snapshot = p2p::Snapshot {
            from_id: net_id::NetworkId(2),
            tick: 1,
            sent_at_ms: 1,
            payload: Scan { x: 5.0, y: 6.0 },
        };
        cmd_tx_b
            .send(p2p::Command::SendSnapshot {
                peer_id: a_peer.clone(),
                snapshot: b_snapshot.clone(),
            })
            .ok();
