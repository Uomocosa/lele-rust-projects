...
mod tests {
    ...
    fn test_usage() {
        ...
        entries.insert(net_id::NetworkId(1), entry(now - 10));
        entries.insert(net_id::NetworkId(2), entry(now - 1000));

        let pruned = prune_stale(entries, now, 300);

        assert_eq!(pruned.len(), 1);
        assert!(pruned.contains_key(&net_id::NetworkId(1)));
        assert!(!pruned.contains_key(&net_id::NetworkId(2)));
    }
}
