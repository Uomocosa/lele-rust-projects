...
use crate::structs::client_error::ClientError as Ce;

pub async fn connect(
    host: &str,
    port: u16,
) -> Result<crate::structs::freenet_client::FreenetClient, Ce> {
    ...
    let (read_tx, read_rx) =
        tokio::sync::mpsc::unbounded_channel::<Result<HostResponse, FcError>>();
    ...
    tokio::spawn(async move {
        while let Some(Ok(msg)) = ws_read.next().await {
            match msg {
                Message::Binary(data) => {
                    match bincode::deserialize::<Result<HostResponse, FcError>>(&data) {
                        Ok(result) => {
    ...
    Ok(crate::structs::freenet_client::FreenetClient {
        write: write_tx,
        read: read_rx,
    })
}
