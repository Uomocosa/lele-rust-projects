...
/// Command-line arguments for `freenet-libp2p-bevy-example-3`.
#[derive(clap::Parser)]
#[command(name = "freenet-libp2p-bevy-example-3")]
pub struct Cli {
    /// Directory holding this instance's identity keypair (default:
    /// `$HOME/.local/share/freenet-libp2p-bevy-example-3`). Two instances started on one
