...
/// Positions every box purely from the engine's snapshot so the display app never runs its own
/// physics. Creates a visual box for each engine body (marking the local player) and despawns any
/// visual box whose engine body is gone.
