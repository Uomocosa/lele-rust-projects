...
fn identity_file_path(dir_override: Option<PathBuf>) -> Option<PathBuf> {
    ...
    std::env::var_os("HOME").map(|home| {
        PathBuf::from(home).join(".local/share/freenet-libp2p-bevy-example-3/identity.bin")
    })
}
...
mod tests {
    ...
    fn test_usage() {
        let dir = std::env::temp_dir().join(format!(
            "freenet_libp2p_bevy_example_3_test_{}",
            std::process::id()
        ));
    ...
    fn different_dirs_yield_different_identities() {
        let dir_a = std::env::temp_dir().join(format!(
            "freenet_libp2p_bevy_example_3_test_a_{}",
            std::process::id()
        ));
        let dir_b = std::env::temp_dir().join(format!(
            "freenet_libp2p_bevy_example_3_test_b_{}",
            std::process::id()
        ));
    ...
    fn concurrent_reads_see_a_stable_on_disk_identity() {
        let dir = std::env::temp_dir().join(format!(
            "freenet_libp2p_bevy_example_3_race_{}",
            std::process::id()
        ));
