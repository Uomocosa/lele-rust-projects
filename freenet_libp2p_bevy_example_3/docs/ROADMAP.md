...
Phased plan to implement example_3's deterministic-lockstep netcode (currently docs-only; the
...
- The `mainnet_automation`-style driver can later drive N real engine builds on the mainnet and
  send a screen video + report to Telegram (example_2 did this for membership; example_3 would
...
- Input-source attestation (trusted hardware / nominated authority) — explicitly scoped out of
  example_3 ([[DIFFERENTIATION]] non-goals).