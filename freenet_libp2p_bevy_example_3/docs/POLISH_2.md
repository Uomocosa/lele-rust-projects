...
2. **3+ consecutive** `mainnet_automation_3` end-to-end passes (no flakes).
...
`mainnet_automation_3/src/new_contract_params.rs`, `src/roster/setup_contract.rs`
...
- **Tests:** `integration_tests_3` — two peers converge, a third joins mid-session, assert all
...
- Linux full green + 3+ consecutive `mainnet_automation_3` E2E passes.
...
- **Cross-OS determinism:** run `cross_os_tests_3::engine_determinism` on Linux + Windows; assert
  equal final hash.
- **Two-machine live lockstep:** self-hosted pipeline + `cross_os_tests_3` / `launch_game` on a
