...
use mainnet_automation_3::Error;
use mainnet_automation_3::build_game;
use mainnet_automation_3::build_report;
use mainnet_automation_3::evaluate;
use mainnet_automation_3::find_window_by_pid;
use mainnet_automation_3::finish_record;
use mainnet_automation_3::jump_instance;
use mainnet_automation_3::kill_all_instances;
use mainnet_automation_3::launch_instances;
use mainnet_automation_3::list_windows;
use mainnet_automation_3::load_creds;
use mainnet_automation_3::move_instance;
use mainnet_automation_3::new_run_dir;
use mainnet_automation_3::parse_config;
use mainnet_automation_3::send_text;
use mainnet_automation_3::send_video;
use mainnet_automation_3::start_record;
use mainnet_automation_3::tile_windows;
use mainnet_automation_3::wait_all_converged;
use mainnet_automation_3::window_info::WindowInfo;

const MOVE_HOLD_MS: u64 = 1500;
const MOVE_PAUSE_MS: u64 = 400;
const JUMP_HOLD_MS: u64 = 90;
...
fn run() -> Result<(), Error> {
    ...
    for (_, win) in &found {
        ...
        jump_instance::jump_instance(&win.id, JUMP_HOLD_MS)?;
        std::thread::sleep(std::time::Duration::from_millis(MOVE_PAUSE_MS));
...
// needed helper:
fn find_windows(
    instances: &[mainnet_automation_3::instance::Instance],
...
fn record_secs(instances: usize) -> u64 {
    let per = 2 * MOVE_HOLD_MS + 3 * MOVE_PAUSE_MS + JUMP_HOLD_MS;
    let drive_ms = instances as u64 * per;
