...
async fn e2e_three_node_production_sync() -> Result<(), Box<dyn std::error::Error>> {
    ...
    testing_2::check_internet_access().await.map_err(|e| {
        format!(
    ...
    let mut app_a = testing_2::ProductionGameApp::new(&wasm, &params, 0).await;
    tokio::time::sleep(Duration::from_secs(JOIN_STAGGER_SECS)).await;
    let mut app_b = testing_2::ProductionGameApp::new(&wasm, &params, 1).await;
