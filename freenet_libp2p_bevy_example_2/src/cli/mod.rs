...
/// Command-line arguments for `freenet-libp2p-bevy-example-2`.
#[derive(clap::Parser)]
#[command(name = "freenet-libp2p-bevy-example-2")]
pub struct Cli {
    /// Directory holding this instance's identity keypair (default:
    /// `$HOME/.local/share/freenet-libp2p-bevy-example-2`). Two instances started on one
