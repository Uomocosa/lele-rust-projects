...
async fn two_node_roster() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (_, gateway_id, gateway_entry) = testing_2::new_identity("gateway-peer");
    let (_, peer_id, peer_entry) = testing_2::new_identity("joining-peer");

    let (mut gateway_client, gateway_roster) =
        testing_2::deploy_roster(gateway.port(), &wasm, &params, gateway_id, gateway_entry)
            .await
    ...
    let (peer_client, peer_roster) =
        testing_2::deploy_roster(peer.port(), &wasm, &params, peer_id, peer_entry)
            .await
            .map_err(|e| format!("peer should deploy/join the roster contract: {e}"))?;
    drop(peer_client);

    let gateway_view =
        testing_2::wait_for_roster_len(&mut gateway_client, 2, Duration::from_secs(60))
            .await
