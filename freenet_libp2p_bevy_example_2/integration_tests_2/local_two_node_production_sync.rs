...
async fn local_two_node_production_sync() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut host = testing_2::ProductionGameApp::new_local(&wasm, &params, 0, None).await;
    let gateway = host.freenet_gateway();

    let mut guest = testing_2::ProductionGameApp::new_local(&wasm, &params, 1, Some(gateway)).await;
