...
// needed helper:
fn trace_entry_changes(
    seen: &mut BTreeMap<boxes::PlayerId, u64>,
    known: &roster::RosterState,
    now: u64,
) {
    for (id, entry) in known {
        let prev = seen.get(id).copied().unwrap_or(0);
        if entry.seq > prev {
            seen.insert(*id, entry.seq);
            let latency_secs = now.saturating_sub(entry.seq);
            tracing::info!(
                target: "roster::change",
                id = %hex::encode(&id[..4]),
                seq = entry.seq,
...
async fn run_roster_loop(
    ...
    own_keypair: libp2p::identity::Keypair,
    own_id: boxes::PlayerId,
    ...
    let mut seen: BTreeMap<boxes::PlayerId, u64> = BTreeMap::new();
    let mut diag_deadline =
        tokio::time::Instant::now() + Duration::from_secs(roster::ROSTER_DIAG_INTERVAL_SECS);
    tracing::info!(
        target: "roster",
        contract_id = %contract_key.encoded_contract_id(),
        code_hash = %contract_key.encoded_code_hash(),
    ...
    loop {
        ...
        let diag_wait = tokio::time::sleep_until(diag_deadline);
        ...
        tokio::select! {
            recv = client.recv() => match recv {
                Ok(HostResponse::ContractResponse(ContractResponse::UpdateNotification {
                    ...
                    if let Some(entries) = roster::decode_roster_update(&update) {
                        ...
                        trace_entry_changes(&mut seen, &known, now);
                        tracing::info!(
                            target: "roster",
                            incoming = %incoming,
                            digest = %roster::digest(&known),
                ...
                Ok(HostResponse::QueryResponse(QueryResponse::NodeDiagnostics(diag))) => {
                    let net = diag
                        .network_info
                        .as_ref()
                        .map(|n| (n.active_connections, n.connected_peers.len()));
                    let sys = diag.system_metrics.as_ref().map(|s| s.hosting_contracts);
                    let instances = diag
                        .contract_states
                        .get(&contract_key.encoded_contract_id())
                        .map(|s| (s.subscribers, s.subscriber_peer_ids.len()))
                        .unwrap_or((0, 0));
                    tracing::info!(
                        target: "freenet::diag",
                        active = net.map(|(a, _)| a).unwrap_or(0),
                        connected_peers = net.map(|(_, c)| c).unwrap_or(0),
                        hosting_contracts = sys.unwrap_or(0),
                        contract_states = diag.contract_states.len(),
                        subscriber_count = instances.0,
                        subscriber_peers = instances.1,
                        subscriptions = diag.subscriptions.len(),
                        uptime_secs = diag
                            .node_info
                            .as_ref()
                            .map(|n| n.uptime_seconds)
                            .unwrap_or(0),
                        "node diagnostics sample"
                    );
                }
                Ok(_) => continue,
                Err(_) => break,
            ...
            _ = &mut diag_wait => {
                let config = NodeDiagnosticsConfig::for_update_propagation_debugging(contract_key);
                if client
                    .send(ClientRequest::NodeQueries(NodeQuery::NodeDiagnostics { config }))
                    .await
                    .is_err()
                {
                    tracing::warn!(target: "freenet::diag", "diagnostics query failed");
                }
                diag_deadline = tokio::time::Instant::now()
                    + Duration::from_secs(roster::ROSTER_DIAG_INTERVAL_SECS);
            }
        }
    }
}
...
pub async fn connect_client_loop(
    ...
    loop {
        ...
        entry.seq = now_unix_secs();
        entry.signature = roster::sign_entry(&own_keypair, &entry.peer_id, &entry.addrs, entry.seq);
