...
mod tests {
    ...
    #[test]
    fn test_usage() {
        let command = engine::EngineCmd::Spawn([1; 32]);
        assert!(matches!(command, engine::EngineCmd::Spawn(_)));
        let action = engine::Action {
            direction: engine::Direction::Right,
            jump: false,
        };
        let _ = engine::EngineCmd::Step {
            tick: 1,
            actions: vec![([1; 32], action)],
        };
        let _ = engine::EngineCmd::Predict {
            inputs: vec![([1; 32], action)],
        };
        let _ = engine::EngineCmd::GetSimState;
    }
}
