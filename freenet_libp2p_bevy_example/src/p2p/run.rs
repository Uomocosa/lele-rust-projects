    ...
    loop {
        ...
        tokio::select! {
            ...
            cmd = cmd_rx.recv() => match cmd {
                ...
                Some(p2p::Command::RequestSnapshot { peer_id, player_id }) => {
                    if let Ok(pid) = peer_id.parse::<PeerId>() {
                        tracing::info!(
                            target: "p2p",
                            peer = %pid,
                            "sending RequestSnapshot"
                        );
                        swarm
                            .behaviour_mut()
                            .netcode
                            .send_request(&pid, p2p::NetcodeMsg::RequestSnapshot { player_id });
                    }
                }
                None => break,
            },
