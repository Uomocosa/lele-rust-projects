...
/// Command-line arguments for `freenet-libp2p-bevy-example`.
#[derive(clap::Parser)]
#[command(name = "freenet-libp2p-bevy-example")]
pub struct Cli {
    /// Directory holding this instance's identity keypair (default:
    /// `$HOME/.local/share/freenet-libp2p-bevy-example`). Two instances started on one
