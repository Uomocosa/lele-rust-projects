...
Complete handoff plan to build the **final example_4 app**: deterministic-lockstep netcode with
...
Build `freenet_libp2p_bevy_example_4`: a peer-to-peer game where every client runs the **same
...
| Test harness | mirror example_2 → `testing_4`, `integration_tests_4`, `cross_os_tests_4`, `mainnet_automation_4` | |
| Names | crate `bevy_freenet_4`, lib `freenet_libp2p_bevy_example_4_lib`, bin `freenet-libp2p-bevy-example-4`, own `contract` | |
...
```
freenet_libp2p_bevy_example_4/
  Cargo.toml                 # workspace: ., testing_4, integration_tests_4, cross_os_tests_4, mainnet_automation_4 ; exclude contract
  ...
  testing_4/ integration_tests_4/ cross_os_tests_4/ mainnet_automation_4/
```
...
- **Adapt** the example_2 harness to the new types:
  - `testing_4` `TestNode` (freenet gateway/peer) + `ProductionGameApp` (full startup path);
    `unique_params` → new `Params`; deploy/sign entries.
  - `integration_tests_4`: two-node roster converge; `local_two_node_production_sync_3`.
  - `cross_os_tests_4`: cross-OS probes (peer discovery, movement sync, engine determinism gate) + mainnet coverage via the same production-startup path; all `#[ignore]`d, driven per-machine by CI. (The former `e2e_tests_3` crate was removed — its multi-machine mainnet value lives here and in `mainnet_automation_4`.)
  - `mainnet_automation_4`: drive N real instances + Telegram video.
- **Add**, per the plan:
...
cargo run --manifest-path ../lele_lint/Cargo.toml                      # from the example_4 root
