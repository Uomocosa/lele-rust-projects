...
async fn two_node_roster() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let gateway = testing_4::TestNode::start_gateway(0)
        .await
        .map_err(|e| format!("gateway node should start: {e}"))?;

    let peer = testing_4::TestNode::start_peer(gateway.public_port(), gateway.public_key_hex())
        .await
        .map_err(|e| format!("peer node should join the gateway: {e}"))?;

    let wasm = testing_4::load_wasm();

    let (_, gateway_id, gateway_entry) = testing_4::new_identity("gateway-peer");
    let (_, peer_id, peer_entry) = testing_4::new_identity("joining-peer");

    let (mut gateway_client, gateway_roster) =
        testing_4::deploy_roster(gateway.port(), &wasm, &params, gateway_id, gateway_entry)
            .await
    ...
    let (peer_client, peer_roster) =
        testing_4::deploy_roster(peer.port(), &wasm, &params, peer_id, peer_entry)
            .await
            .map_err(|e| format!("peer should deploy/join the roster contract: {e}"))?;
    drop(peer_client);

    let gateway_view =
        testing_4::wait_for_roster_len(&mut gateway_client, 2, Duration::from_secs(60))
            .await
