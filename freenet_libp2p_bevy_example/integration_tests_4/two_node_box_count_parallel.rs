...
use freenet_libp2p_bevy_example_lib::engine;
use libp2p::identity::Keypair;

// needed helper:
fn assert_boxes(app: &mut testing_4::TestGameApp) -> Result<(), String> {
    ...
    assert!(
        (remotes[0].1.y - (engine::GROUND_TOP + engine::BOX_SIZE / 2.0)).abs() < 10.0,
        "remote box must rest on the ground"
    );
...
async fn two_node_box_count_parallel() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut gateway_app = testing_4::TestGameApp::new(
        gateway.port(),
        &wasm,
        &params,
    ...
    let mut peer_app = testing_4::TestGameApp::new(
        peer.port(),
        &wasm,
        &params,
        Keypair::generate_ed25519(),
